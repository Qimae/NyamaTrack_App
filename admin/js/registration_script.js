...
// Verify account code on page load
async function verifyAccountCode() {
    const urlParams = new URLSearchParams(window.location.search);
    const code = urlParams.get('code');
    
    if (!code) {
        window.location.href = 'index.php?error=no_code';
        return false;
    }

    try {
        const response = await fetch('api/verify_registration_access.php', {
            method: 'POST',
            headers: {
        ...
        if (!data.success) {
            await Swal.fire({
                title: 'Access Denied',
                text: data.message || 'Invalid or expired registration code',
                icon: 'error',
        ...
        await Swal.fire({
            title: 'Error',
            text: 'An error occurred while verifying your access. Please try again.',
            icon: 'error',
            confirmButtonColor: '#e53e3e',
            background: '#1a1a1a',
            color: '#fff',
...
function changeStep(direction) {
    ...
    // Only validate when moving forward and not on the last step
...
document.addEventListener('DOMContentLoaded', async function () {
    // Verify account code before showing the form
    const isVerified = await verifyAccountCode();
    if (!isVerified) return;
    
    // Initialize buttons and progress
    updateProgress(1);
    updateButtons(1);  // Make sure buttons are in the correct state
